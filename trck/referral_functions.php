...
function getMessageIDDetails($msgID) {
    global $db;
    $response = array();
    $sql = "SELECT * FROM tbl_referral_automations_tracking_logs WHERE message_id='{$msgID}' LIMIT 1";
    $result = $db->query($sql);
    if ($result->num_rows > 0) {
        $response = $result->fetch_assoc();
    }
...
function getBroadcastMessageIDDetails($msgID) {
    global $db;
    $response = array();
    $sql = "SELECT * FROM tbl_broadcast_emails_tracking_logs WHERE message_id='{$msgID}' LIMIT 1";
    $result = $db->query($sql);
    if ($result->num_rows > 0) {
        $response = $result->fetch_assoc();
    }
