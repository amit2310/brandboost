...
//            $('.nav-tabs a[href="#chooseWorkflowEmail"]').tab('hide');
    ...
    $(document).on("click", ".wf_editSMSCampaign", function (e) {
        ...
        if (campaignId != '' && moduleName != '') {
            ...
            frameDoc = frame.contentDocument || frame.contentWindow.document;
            frameDoc.documentElement.innerHTML = "";
            $("#loadstripotemplateInline").attr("src", site_url + 'admin/workflow/loadStripoSMSCampaign/' + moduleName + '/' + campaignId);
            $("#loadstripotemplateInline").attr("height", 900);
            //$("#workflow_template_stripo_modal").modal();
            $("#superContainer").hide();
            $("#editTemplateContainer").show();
            $(".savecloseTemplate").show();
        ...
        return false;
        displayEditCampaignPopup(moduleName, campaignId);
        return false;
    ...
    $(document).on("click", ".wf_previewCampaign", function (e) {
        ...
        if (campaignId != '' && moduleName != '') {
            $.ajax({
                ...
                type: "POST",
                data: {moduleName: moduleName, campaignId: campaignId, moduleUnitId: moduleUnitId, previewType:previewType},
                dataType: "json",
                success: function (data) {
                    if (data.status == 'success') {
    ...
    $(document).on("click", ".wf_deleteCampaign", function (e) {
        e.preventDefault();
...
function displayEditCampaignPopup(moduleName, campaignId) {
    $.ajax({
        ...
        success: function (data) {
            if (data.status == 'success') {
                $('#smsWorkflowCampaignPreview').html(data.compiled_content.replace(/\r\n|\r|\n/g, "<br />"));
                $('#wf_sms_editor_campaign_id').val(campaignId);
                $('#smsWorkflowCampaignBody').attr('created_date', data.createdVal);
                $('#smsWorkflowCampaignBody').val(data.compiled_content.replace(/<br *\/?>/gi, '\n'));
                $('#workflow_sms_template_stripo_modal').modal();
            } else {
                alertMessage('Error: Some thing wrong!');
            }
