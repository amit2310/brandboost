            ...
            <div class="col-md-12">
                <div class="white_box profile_sec mb20">
                    ...
                    <div class="p20">
                        ...
                        <!-- <div class="col-xs-6">
                            <a class="pull-right txt_grey fsize12" href="#"><i><img style="vertical-align: top; margin-top: 4px" src="{{ base_url() }}assets/profile_images/edit_icon.png" width="12"></i> &nbsp; Edit</a>
                        </div> -->
                        </div>
                    </div>
                </div>
                                    ...
                                    <li>
                                        <div class="pull-left">
                                            <div class="media-left"><img
                                                    src="{{ base_url() }}assets/profile_images/user_placeholder.png"/>
                                            </div>
                                            <div class="media-left"><p class="txt_dark">{{ $value->title }}</p></div>
                                        </div>
                                        <div class="pull-right">
                                            <div class="media-left text-left pr40 w100"><p><strong><img class="email"
                                                                                                        src="{{ base_url() }}assets/profile_images/email_icon16.png"/>{{ ucfirst($value->source_type) }}
                                                    </strong></p></div>
                                            <div class="media-left text-left pr30 w100"><p>
                                                    <strong>{{ $value->status == 1 ? '<i class="icon-circle2 txt_green"></i>active' : '<i class="icon-circle2 txt_red"></i>inactive' }}</strong>
                                                </p></div>
                                            <div class="media-left text-left pr30 w100"><p>
                                                    <strong>{{ $createdAt }}</strong></p></div>
                                                        ...
                                                        <li>
                                                            @if($value->status != 1)
                                                                <a href="javascript:void(0);" class="changeStatus"
                                                                   data-status="1" data-id="{{ $value->id }}"><i
                                                                        class="icon-primitive-dot txt_green"></i> Active</a>
                                                            @else
                                                                <a href="javascript:void(0);" class="changeStatus"
                                                                   data-status="0" data-id="{{ $value->id }}"><i
                                                                        class="icon-primitive-dot txt_red"></i> Inactive</a>
                                                            @endif
                                                        </li>
                                                        <li>
                                                            <a href="{{ base_url() }}user/referral/reports/{{ $value->id }}"><i
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('.changeStatus').click(function () {
                ...
                $('.overlaynew').show();
                $.ajax({
                    url: "{{ base_url('user/referral/updateReferralUser') }}",
                    type: "POST",
