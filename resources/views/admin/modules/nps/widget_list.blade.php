            ...
            <div class="col-md-9 text-right btn_area">
                <div class="btn-group">
                    <button type="button" class="btn light_btn btn-icon dropdown-toggle" data-toggle="dropdown">
                        <i class="icon-calendar2"></i>&nbsp; &nbsp; Filter NPS Widget &nbsp; &nbsp; <span class="caret"></span>
					</button>
						...
						<div class="panel-heading">
							...
							<div class="heading-elements">
								...
								<div class="table_action_tool">
									...
									<button id="deleteNPSWidget" class="btn btn-xs custom_action_box"><i class="icon-trash position-left"></i> Delete</button>
									<button id="archiveButtonNPSWidget" class="btn btn-xs custom_action_box"><i class="icon-gear position-left"></i> Archive</button>
								</div>
							...
							<table class="table datatable-sorting-new" id="npsWidgetList">
								<thead>
									<tr>
										...
										<th><i class="icon-stack-star"></i>NPS Widget Title</th>
										<th><i class="icon-stack-star"></i>NPS Name</th>
								...
								<tbody>
									<?php
                                        ...
                                        foreach ($oWidgetsList as $data) {
                                            $npsData = $this->mNPS->getNps($currentUserId, $data->nps_id);
											//pre($npsData);
										?>
...
<div id="addNPSWidgetModal" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="post" name="frmAddNPSWidget" id="frmAddNPSWidget" action="javascript:void();">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h5 class="modal-title">Add NPS Widget</h5>
				</div>
				
                <div class="modal-body">
                    <p>Please Enter Title below:</p>
                    <input class="form-control" id="npsTitle" name="npsTitle" placeholder="Enter Title" type="text" required>
				</div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
		...
		var tableId = 'npsWidgetList';
		var tableBase = custom_data_table(tableId); 
		
		// Setup - add a text input to each footer cell
		
		$('#npsWidgetList thead tr').clone(true).appendTo('#npsWidgetList thead');        
		$('#npsWidgetList thead tr:eq(1) th').each(function (i) {  
        ...
        $(document).on('click', '#deleteNPSWidget', function () {
		...
		$(document).on('click', '#archiveButtonNPSWidget', function () {
        ...
        $('#addNPSWidget').click(function () {
            $('#addNPSWidgetModal').modal();
		});
		
        $('#frmAddNPSWidget').on('submit', function (e) {
            $('.overlaynew').show();
            var npsTitle = $('#npsTitle').val();
            $.ajax({
                url: '<?php echo base_url('admin/modules/nps/addNPSWidget'); ?>',
                type: "POST",
                data: {'npsTitle': npsTitle},
                dataType: "json",
                success: function (data) {
                    if (data.status == 'success') {
                        $('.overlaynew').hide();
