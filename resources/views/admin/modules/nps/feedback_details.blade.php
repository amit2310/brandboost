  ...
  <!--&&&&&&&&&&&& PAGE HEADER END&&&&&&&&&&-->
	...
	<div class="row">
								...
								if (!empty($mediaArray)) {
									foreach ($mediaArray as $key => $data) :
										...
										if ($data['media_type'] == 'image') {
											if ($key == 0) {
												@endphp
												<div class="col-xs-12 col-md-12"> <a href="https://s3-us-west-2.amazonaws.com/brandboost.io/campaigns/{{ $data['media_url'] }}" class="thumbnail2"> <img class="big" src="https://s3-us-west-2.amazonaws.com/brandboost.io/campaigns/{{ $data['media_url'] }}" alt=""> </a> </div>
											@php } @endphp
											<div class="col-xs-6 col-md-4"> <a href="https://s3-us-west-2.amazonaws.com/brandboost.io/campaigns/{{ $data['media_url'] }}" class="thumbnail2"> <img src="https://s3-us-west-2.amazonaws.com/brandboost.io/campaigns/{{ $data['media_url'] }}" alt="" class=" small"> </a> </div>
											@php
										} 
									endforeach;
								}  else {
											...
											if ($data['media_type'] == 'video') {
												...
												<div class="embed-responsive embed-responsive-16by9 mb-20">
													<video width="320" height="240" controls>
														<source src="https://s3-us-west-2.amazonaws.com/brandboost.io/campaigns/{{ $data['media_url'] }}" type="video/{{ $ext }}">
													</video>
												</div>
												...
												<div class="viddetails">
													...
													<p><span>Download Video:</span> <a style="text-decoration: underline;" href="https://s3-us-west-2.amazonaws.com/brandboost.io/campaigns/{{ $data['media_url'] }}" download="video">Download Video</a></p>
												</div>
											@php
											}
										}
		  ...
		  <div class="col-md-6">
			  ...
			  <div class="panel-heading">
				<h6 class="panel-title">NPS Feedback For {{ $oScore->npstitle != ''? $oScore->npstitle: '' }}</h6>
				<div class="heading-elements"><a style="cursor: pointer;"><i class="icon-more2"></i></a></div>
			  </div>
			  <div class="panel-body br0">
			  <p class="fsize14 mb20">{{ $oScore->title != ''? $oScore->title: 'N/A' }}</p>
			  ...
			  <div class="media-left media-middle"> <a class="icons" style="cursor: pointer;"><img src="{{ $avatarImage }}" class="img-circle img-xs" alt=""></a> </div>
			  <div class="media-left">
				  <div class="text-muted pt-5">by {{ $oScore->firstname." ".$oScore->lastname }}  <span class="ml20"><i class="icon-checkmark3 fsize12"></i>&nbsp; Verified Purchase</span></div>
				</div>
			  </div>
			  <div class="panel-footer p20 ">
			  <p class="mb0 fsize13"> {{ $oScore->feedback != ''? $oScore->feedback: 'N/A' }} </p>
			  </div>
			</div>
		  </div>
			  ...
			  <div class="panel-body p0" >
				<div class="interactions p25">
					<ul>
						...
						<li><small>Name</small> <strong>{{ $oScore->firstname.' '.$oScore->lastname }}</strong></li>
						<li><small>Email</small> <strong>{{ $oScore->email }}</strong></li>
						<li><small>Phone</small> <strong>{{ isset($oScore->mobile) ? $oScore->mobile : 'N/A' }}</strong></li>
					</ul>
				</div>
				...
				@foreach ($oNotes as $key => $noteData)
					<div class="p25 bbot">
						...
						<div class="text-right">
							<a href="javascript:void(0)" class="editNote" noteid="{{ $noteData->id }}"> <span class="label addtag bkg_grey txt_white br5"> Modify</span></a>
							<a href="javascript:void(0)" class="deleteNote" noteid="{{ $noteData->id }}"> <span class="label addtag bkg_red txt_white br5"> Delete</span></a>
						</div>
					</div>
                @endphp
				
				<div class="p25 btop">
				<button data-toggle="modal" data-target="#addnotes" type="button" class="btn dark_btn btn-xs mr20">Add Note</button>
				</div>
			  </div>
			</div>
		  </div>
...
<div id="ReviewTagListModal" class="modal fade">
  ...
  <script>
    ...
    function loadMoreComments(reviewID, startinglimitVal) {
        $('.overlaynew').show();
        $.ajax({
            url: "{{ base_url('admin/brandboost/getreviwecomments/') }}",
            type: "POST",
    ...
    function saveCommentLikeStatus(commentID, statusType) {
        ...
        $.ajax({
            ...
            type: "POST",
            data: {_token: '{{csrf_token()}}', 'commentId': commentID, 'status': statusType,_token: '{{csrf_token()}}'},
            dataType: "json",
            success: function (data) {
                $('.overlaynew').hide();
                if (data.status == 'success') {
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.editNote', function () {
            ...
            $.ajax({
                url: "{{ base_url('admin/modules/nps/getNPSNoteById') }}",
                type: "POST",
        ...
        $(document).on('click', '.deleteNote', function () {
            ...
            if (conf == true) {
                var noteId = $(this).attr('noteid');
                $.ajax({
                    url: "{{ base_url('admin/modules/nps/deleteNPSNote') }}",
                    type: "POST",
                    data: {_token: '{{csrf_token()}}', noteid: noteId},
                    dataType: "json",
                    success: function (data) {
        ...
        $("#updateNote").submit(function () {
            ...
            $.ajax({
                url: "{{ base_url('admin/modules/nps/updatNotes') }}",
                type: "POST",
                data: formData,
                contentType: false,
                cache: false,
        ...
        $(document).on("click", "#saveNPSNotes", function () {
            ...
            $('.overlaynew').show();
            $.ajax({
                url: "{{ base_url('/admin/modules/nps/saveNPSNotes') }}",
                type: "POST",
                data: formdata,
                dataType: "json",
        ...
        $(document).on('click', '#loadMoreComment', function(){
	        ...
	        $.ajax({
	            url: "{{ base_url('admin/brandboost/loadComment') }}",
	            type: "POST",
