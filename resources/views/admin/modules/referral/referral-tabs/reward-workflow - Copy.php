...
<style>
    ...
    .insert_tag_button {margin: 0 8px 10px 0; font-size: 10px; padding: 5px;}
        ...
        <div class="timeline-container"> 
            ...
            <?php
				...
				foreach ($eventsData as $key => $eventData) {
					...
					if ($eventData->event_type == 'invite-email' || $eventData->event_type == 'invite-sms' || $eventData->event_type == 'invite-email-reminder' || $eventData->event_type == 'invite-sms-reminder') {
						$campaignArray = $this->mReferral->getCampaignsByEventID($eventData->id);
						...
						<?php
							foreach ($campaignArray as $campaignData) {
								...
								if ($campaignData->campaign_type == 'Email') {
									//pre($campaignData);
									$aStats = $this->mReferral->getReferralSendgridStats('campaign', $campaignData->id);
											...
											<div class="panel-body">
												<div class="row">
													...
													<div class="col-md-5 text-right">
														...
														<?php if($eventData->event_type == 'invite-email-reminder') { ?>
															<div class="row" style="margin-top:10px;">
																	Reminder Loop: <input type="text" class="form-control saveReminderLoop" event_id="<?php echo $eventData->id; ?>" value="<?php echo $eventData->reminder_loop; ?>" style="width: 40px; display: inline; height: 37px; padding: 5px; text-align:center;font-weight: bold; color: #F00;">
								...
								<?php
									...
									$aStatsSms = $this->mReferral->getEmailTwilioStats('campaign', $campaignData->id);
									$aCategorizedStatsSms = $this->mReferral->getEmailTwilioCategorizedStatsData($aStatsSms);
								...
								<div>
									<?php /* ?><div class="timeline-date text-muted">
										<button class="btn bl_cust_btn btn-success waitTime" id="waitTime_<?php echo $eventData->id; ?>" event_id="<?php echo $eventData->id; ?>" delay_value="<?php echo $eventDataArray->delay_value == '' ? '10' : $eventDataArray->delay_value; ?>" delay_unit="<?php echo $eventDataArray->delay_unit == '' ? 'minute' : $eventDataArray->delay_unit; ?>"><i class="fa fa-clock-o"></i> &nbsp; <span class="text-semibold">Wait for <?php echo $eventDataArray->delay_value == '' ? '10' : $eventDataArray->delay_value; ?> <?php echo $eventDataArray->delay_unit == '' ? 'minute' : $eventDataArray->delay_unit; ?>(s)</span></button>
									</div><?php */ ?>
												...
												<div class="panel-body">
													<div class="row">
														...
														<div class="col-md-5 text-right">
															...
															<?php endif; ?>
															<button class="btn red" onclick="javascript:delete_campaign('<?php echo $campaignData->id; ?>');"><i class="fa fa-trash"></i></button>
															<?php if($eventData->event_type == 'invite-sms-reminder') { ?>
																<div class="row" style="margin-top:10px;">
																	Reminder Loop: <input type="text" class="form-control saveReminderLoop" event_id="<?php echo $eventData->id; ?>" value="<?php echo $eventData->reminder_loop; ?>" style="width: 40px; display: inline; height: 37px; padding: 5px; text-align:center;font-weight: bold; color: #F00;">
																	<button class="btn red saveReminderCampaign" reminder_loop_status="start" id="reminderCampaignStart_<?php echo $eventData->id; ?>" style="margin-right:10px; <?php echo $eventData->reminder_loop_status  == 'start' ? 'display:none;' : ''; ?>" event_id="<?php echo $eventData->id; ?>"><i class="fa fa-play"></i></button>
																	<button class="btn bl_cust_btn saveReminderCampaign" reminder_loop_status="pause" id="reminderCampaignPause_<?php echo $eventData->id; ?>" style="margin-right:10px; <?php echo $eventData->reminder_loop_status  == 'start' ? '' : 'display:none;'; ?>" event_id="<?php echo $eventData->id; ?>"><i class="fa fa-pause"></i></button>
			...
			<!-- Task  SMS 1-->
			<div class="timeline-date text-muted">
				<div class="trigger_btn_outer">
					<button class="btn trigger_btn btn-success mb-20"><i class="fa fa-shield"></i> &nbsp; Post Sale</button>
				</div>
			</div>
            ...
            <?php
				...
				foreach ($eventsData as $key => $eventData) {
					$eventDataArray = json_decode($eventData->data);
					if ($eventData->event_type == 'sale-email' || $eventData->event_type == 'sale-sms' || $eventData->event_type == 'sale-email-reminder' || $eventData->event_type == 'sale-sms-reminder') {
						$campaignArray = $this->mReferral->getCampaignsByEventID($eventData->id);
						if (!empty($campaignArray)) {
						?>
						<div class="timeline-row post-full">
									...
									<div class="timeline-content">
										<div class="panel border-left-lg gery_bkg border-left-primary" style="<?php echo $campaignData->status == 'draft' ? 'background:#f2bfbf' : ''; ?>">
											<div class="panel-body">
												<div class="row">
													<div class="col-md-7">
														<h6 class="no-margin-top"><a href="javascript:void(0);"><?php echo $saleCampaignNo; ?>. <?php echo $campaignData->name; ?> : <?php echo $campaignData->subject; ?></a></h6>
													</div>
													<div class="col-md-5 text-right">
														<a class="btn bl_cust_btn  edit_email_template editEmailTemplate" campaign_id="<?php echo $campaignData->id; ?>" id="editEmailTemplate_<?php echo $campaignData->id; ?>" style="cursor: pointer;" ><i class="fa fa-edit"></i> </a>
														<a class="btn green_cust_btn showContentPreview" href="javascript:void(0);" campaign_type="email" campaign_id="<?php echo $campaignData->id; ?>"><i class="fa fa-eye"></i> </a>
														...
														<?php endif; ?>
														<button class="btn red" onclick="javascript:delete_campaign('<?php echo $campaignData->id; ?>');"><i class="fa fa-trash"></i></button>
														<?php if($eventData->event_type == 'sale-email-reminder') { ?>
															<div class="row" style="margin-top:10px;">
																Reminder Loop: <input type="text" class="form-control saveReminderLoop" event_id="<?php echo $eventData->id; ?>" value="<?php echo $eventData->reminder_loop; ?>" style="width: 40px; display: inline; height: 37px; padding: 5px; text-align:center;font-weight: bold; color: #F00;">
																<button class="btn red saveReminderCampaign" reminder_loop_status="start" id="reminderCampaignStart_<?php echo $eventData->id; ?>" style="margin-right:10px; <?php echo $eventData->reminder_loop_status  == 'start' ? 'display:none;' : ''; ?>" event_id="<?php echo $eventData->id; ?>"><i class="fa fa-play"></i></button>
																<button class="btn bl_cust_btn saveReminderCampaign" reminder_loop_status="pause" id="reminderCampaignPause_<?php echo $eventData->id; ?>" style="margin-right:10px; <?php echo $eventData->reminder_loop_status  == 'start' ? '' : 'display:none;'; ?>" event_id="<?php echo $eventData->id; ?>"><i class="fa fa-pause"></i></button>
												...
												<div class="panel-body">
													<div class="row">
														...
														<div class="col-md-5 text-right">
															...
															<?php endif; ?>
															<button class="btn red" onclick="javascript:delete_campaign('<?php echo $campaignData->id; ?>');"><i class="fa fa-trash"></i></button>
															<?php if($eventData->event_type == 'sale-sms-reminder') { ?>
																<div class="row" style="margin-top:10px;">
																	Reminder Loop: <input type="text" class="form-control saveReminderLoop" event_id="<?php echo $eventData->id; ?>" value="<?php echo $eventData->reminder_loop; ?>" style="width: 40px; display: inline; height: 37px; padding: 5px; text-align:center;font-weight: bold; color: #F00;">
																	<button class="btn red saveReminderCampaign" reminder_loop_status="start" id="reminderCampaignStart_<?php echo $eventData->id; ?>" style="margin-right:10px; <?php echo $eventData->reminder_loop_status  == 'start' ? 'display:none;' : ''; ?>" event_id="<?php echo $eventData->id; ?>"><i class="fa fa-play"></i></button>
																	<button class="btn bl_cust_btn saveReminderCampaign" reminder_loop_status="pause" id="reminderCampaignPause_<?php echo $eventData->id; ?>" style="margin-right:10px; <?php echo $eventData->reminder_loop_status  == 'start' ? '' : 'display:none;'; ?>" event_id="<?php echo $eventData->id; ?>"><i class="fa fa-pause"></i></button>
			...
			?>
			<?php if($saleCampaignNo < 4){ ?>
				<div class="timeline-row post-full">
					<div class="timeline-icon"> <a  onclick="openCampaignBox('sale');" href="javascript:void(0);" class="circle_plus"><i class="icon-plus3"></i></a> </div>
				</div>
			<?php } ?>
		</div>
	</div>
...
<script>
	function openCampaignBox(campaignType){
		if(campaignType == 'invite'){
			$('.Email_campaign.Sale, .Sale.Reminder_campaign').hide();
			$('.Email_campaign.Invite, .Invite.Reminder_campaign').show();
			$('.SMS_campaign.Sale, .Sale.Reminder_campaign').hide();
			$('.SMS_campaign.Invite, .Invite.Reminder_campaign').show();
		}else{
			$('.Email_campaign.Sale, .Sale.Reminder_campaign').show();
			$('.Email_campaign.Invite, .Invite.Reminder_campaign').hide();
			$('.SMS_campaign.Sale, .Sale.Reminder_campaign').show();
			$('.SMS_campaign.Invite, .Invite.Reminder_campaign').hide();
		}
		$('#addnewaction').modal();
	}
	...
	function delete_campaign(campaignId){
		...
		function (isConfirm) {
			if (isConfirm) {
				$('.overlaynew').show();
				$.ajax({
					url: '<?php echo base_url('admin/modules/referral/deleteReferralCampaign'); ?>',
					type: "POST",
	...
	$(document).ready(function () {
		...
		$(document).on("click", ".showErrorMsg", function () {
			alertMessage('This referral campaign has been already added.');
		});
		...
		$('.saveReminderLoop').keyup(function(){
			...
			$.ajax({
				url: '<?php echo base_url('admin/modules/referral/updateReferralReminderLoop'); ?>',
				type: "POST",
		...
		$('.saveReminderCampaign').click(function(){
			...
			$.ajax({
				url: '<?php echo base_url('admin/modules/referral/updateReferralReminderCampaign'); ?>',
				type: "POST",
		...
		$(document).on("click", ".editEmailTemplate", function () {
			...
			$.ajax({
				url: '<?php echo base_url('admin/modules/referral/getReferralCampaign'); ?>',
				type: "POST",
				data: {campaign_id: campaignId},
				dataType: "json",
				success: function (data) {
					if (data.status == 'success') {
						...
						$('#campaignTempBody').val(data.content);
		...
		$('.editSmsTemplate').click(function () {
			...
			$.ajax({
				...
				success: function (data) {
					if (data.status == 'success') {
						var dataVal = data.campData;
						$('#smsCampaignTempBody').val(data.content);
		...
		$('.eventTimeUpdate').on('submit', function (e) {
			...
			$.ajax({
				url: '<?php echo base_url('admin/modules/referral/updateReferralEvent'); ?>',
				type: "POST",
		...
		$('#updateEmailCampaign').on('click', function (e) {
			...
			$.ajax({
				url: '<?php echo base_url('admin/modules/referral/updateReferralCampaign'); ?>',
				type: "POST",
