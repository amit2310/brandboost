						...
						<div class="p20 bbot pt10 pb10">
							...
							<div class="tdropdown ml0">
								...
								<ul style="right: 0px!important; margin-top: 25px; left: -20px;" class="dropdown-menu dropdown-menu-left chat_dropdown">
									...
									<li><strong><a href="javascript:void(0)"><img class="small" src="/assets/images/cd_icon2.png"/><b class=" Smallwebfavtab">Favorites (<?php echo $FavUsers; ?>)  </b></a></strong>
									</li>
...
<script>
		...
		setTimeout(function(){ $('#preview_image_notes').trigger('click');}, 100);
		});
		//  ######### media  file upload ############ //

	});
	
	$(document).on('change', '#preview_image_notes', function(e) {
		$('.overlaynew').show();
		...
		var newToken =  $('#preview_image_notes').attr('newToken');
			...
			success => {
				...
				$('#preview_image_notes').remove();
                $('.file_sms_web').append('<input style="display:none;" id="preview_image_notes" newToken="" user_id="" type="file">');
			} 
			).catch(
