...
<div class="form-group">
    <label>Subject: </label>
    <input class="form-control" id="wf_template_subject" name="wf_template_subject" placeholder="Email Subject"
           type="text" value="{{ $subject }}" style="width:100%">
</div>
@if($moduleName != 'automation')
    <div class="form-group">
        <label>Greetings: </label>
        <input type="text" required="required" name="greeting" id="wfEmailTemplateGreetings" class="form-control"
               placeholder="Greetings" value="{{ $greeting }}" style="width:100%"/>
    </div>

    <div class="form-group">
        <label>Introduction: </label>
        <textarea style="height: 100px;" name="introduction" id="wfEmailTemplateIntroduction" class="form-control"
                  placeholder="Introduction" style="width:100%">{{ $introduction }}</textarea>
    </div>
@endif
@if($moduleName == 'automation')
    <div>
...
<script type="application/javascript" src="{{ base_url() }}assets/js/notifications.js"></script>
<script>
    // Utility methods
    ...
    function loadDemoTemplate(callback) {
        ...
        request('GET', '{{ base_url() }}admin/workflow/loadStripoTemplateResources/html/{{ $moduleName }}/{{ $templateID }}', null, function (html) {
            request('GET', '{{ base_url() }}admin/workflow/loadStripoTemplateResources/css/{{ $moduleName }}/{{ $templateID }}', null, function (css) {
                callback({html: html, css: css});
            });
        });
    }
            ...
            window.Stripo.init({
                    ...
                    mergeTags: [
                        {
                            ...
                            entries: [
                                    ...
                                    @foreach ($tags as $tag)
                                {
                                    label: '{{ ucwords(strtolower(str_replace(array('{', '}', '_'), array('', '', ' '), $tag))) }}',
                                    value: '{{ $tag }}'
                                },
                                @endforeach
                                @endif
                    ...
                    getAuthToken: function (callback) {
                        request('POST', 'https://plugins.stripo.email/api/v1/auth',
                            JSON.stringify({
                                pluginId: '{{ $this->config->item("stripo_plugin_id") }}',
                                secretKey: '{{ $this->config->item("stripo_secret_key") }}'
                            }),
                            function (data) {
