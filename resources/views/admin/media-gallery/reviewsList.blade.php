        ...
        <td>
            <div class="media-left media-middle">
                <label class="custmo_checkbox pull-left" style="margin-top:5px;">
                    <input type="checkbox" name="reviewsId[]" class="reviewsId"
                           value="{{ $review->id }}" {!! $checkedVal == true ? 'checked' : '' !!} >
                    <span class="custmo_checkmark"></span>
                </label> &nbsp;
                <a class="icons" href="#">
                    <img src="{{ $brand_img }}" onerror="this.src='{{ base_url('assets/images/wakerslogo.png') }}'"
                         class="img-circle br5 img-xs" alt="">
                </a>
            </div>
            <div class="media-left">
                <div class=""><a href="javascript:void(0);"
                                 class="text-default text-semibold">{{ $review->brand_title }}</a></div>
            </div>
        </td>
        <td>
            <div class="media-left media-middle"><a class="icons" href="#"><img src="{{ $mediaImageUrl }}"
                                                                                onerror="this.src='{{ base_url('assets/images/wakerslogo.png') }}'"
                                                                                class="img-circle br5 img-xs"
                                                                                alt=""></a></div>
            <div class="media-left">
                <div class=""><a href="javascript:void(0);"
                                 class="text-default text-semibold">{{ $review->review_title }}</a><br>
                    <div>
                        @php
                            for ($i = 1; $i <= 5; $i++) {
                                if ($i <= $review->ratings) {
