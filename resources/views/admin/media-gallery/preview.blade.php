...
<div class="gallery_slider_widget {{ $mainWigetClassName }} slides{{ ($galleryType == '') ? '' : $galleryType }} imgSize{{ ($imageSize == '') ? 'Medium' : ucfirst($imageSize) }} galleryType @php if($galleryDesign == 'onerow'){ echo ''; }else{ echo $galleryDesign == 'horizontal' ? 3 : 2; } @endphp" id="bbColorOrientationSection">
	<h2 class="reviewTitleBH {{ $allowTitle != '0' ? '' : 'hidden' }}"><strong class="reviewTitle">{{ $name == '' ? 'Gallery' : $name }}</strong> <span>{{ count($reviewIDArray) }} photos</span></h2>
	@if($galleryDesign == 'onerow')
		<div class="arrow reviewArrowBH {{ $allowArrows != '0' ? '' : 'hidden' }}">
			<a href="javascript:void(0);" class="left_arrow" bb_direction="left"><img src="{{ base_url() }}assets/images/widget/arrow-left.png"></a> 
			<a href="javascript:void(0);" class="right_arrow" bb_direction="right"><img src="{{ base_url() }}assets/images/widget/arrow-right.png"></a>
		</div>
		<div style="overflow:hidden; float:left; width:100%;" class="@if($galleryData->allow_border_shadow == 1) borderBoxShadow @endif">
