...
<script type="text/javascript"
        src="{{ base_url() }}assets/js/plugins/editors/wysihtml5/locales/bootstrap-wysihtml5.ua-UA.js"></script>
<script type="text/javascript" src="{{ base_url() }}assets/js/pages/editor_wysihtml5.js"></script>
@php
    $aTags = array('FIRST_NAME', 'LAST_NAME', 'EMAIL', 'WEBSITE');
@endphp
<div class="tab-pane {{ $activeDesign != ''?$activeDesign:'' }}" id="right-icon-tab2">
    <form method="post" name="updateEmailTempaltes" id="updateEmailTempaltes" action="javascript:void(0);" novalidate>
        <div class="snoteeditoe">
            <textarea rows="15" name="emailtemplate" id="emailtemplate" class="wysihtml5 wysihtml5-default form-control"
                      required>{!! base64_decode($oBroadcast->html) !!}</textarea>

            <div class="col-lg-12" style="margin-top:10px;">
                @foreach ($aTags as $value)
                    <button type="button" data-toggle="tooltip" title="Click to insert Tag"
                            data-tag-name="{{ '{'.$value.'}' }}"
                            class="btn btn-default add_btn draggable insert_tag_button">{{ '{'.$value.'}' }}</button>
                    &nbsp;&nbsp;
                @endforeach
            </div>
        </div>
        <div class="col-lg-12 text-right" style="margin-top:30px;">
            <input name="campaignId" id="campaignId" value="{{ $oBroadcast->id }}" type="hidden">
            <input type="hidden" name="tab" value="Settings">
            <button type="button" class="btn bl_cust_btn new btn-default broadcastBackButton" tab="Select List">Back
            </button>
    ...
    $(document).ready(function () {
        ...
        $('#updateEmailTempaltes').on('submit', function () {
            ...
            if (emailtemplate == '') {
                ...
                $.ajax({
                    url: "{{ base_url('admin/broadcast/updateBroadcastTempalte') }}",
                    type: "POST",
                    data: formdata,
                    dataType: "json",
