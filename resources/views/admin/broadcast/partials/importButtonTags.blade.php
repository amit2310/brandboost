@if (!empty($oCampaignLists))
    ...
    <button class="btn btn-xs btn_white_table addtag"><img alt="" src="{{ base_url() }}assets/images/blue_line.png" > {{ $oRec->list_name }}</button>
    @endforeach
@endif
...
if (!empty($oCampaignTags)) {
    ...
    foreach ($aTags as $oTag) {
        if (in_array($oTag->tagid, $aSelectedTags)){
            @endphp
            <button class="btn btn-xs btn_white_table addtag"><img alt="" src="{{ base_url() }}assets/images/blue_hash.png"> {{ $oTag->tag_name }}</button>
            @php
        }
    }
}
