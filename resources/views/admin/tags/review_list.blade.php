...
@php list($canRead, $canWrite) = fetchPermissions('Reviews') @endphp
						...
						<div class="panel-heading"> 
							...
							<div class="heading-elements">
								<div style="display: inline-block; margin: 0;" class="form-group has-feedback has-feedback-left">
									<input class="form-control input-sm cus_search" tableID="onsitereviewReview" placeholder="Search by name" type="text">
										...
										<tr id="append-{{ $oReview->id }}" class="selectedClass">
											...
											<td style="width: 40px!important; display: none;" class="editAction">
												<label class="custmo_checkbox pull-left">
													<input type="checkbox" name="checkRows[]" class="checkRows" id="chk{{ $oReview->id }}" value="{{ $oReview->id }}" >
											...
											<td>
												<div class="media-left media-middle"> 
													<a class="icons" href="#">
														<img src="{{ base_url('profile_images/').$avatarImage }}" class="img-circle img-xs" alt="">
													</a> 
												</div>
												<div class="media-left">
													<div class="pt-5">
														<a href="#" class="text-default text-semibold">
															...
															<img class="flags" src="{{ base_url() }}assets/images/flags/ao.png">
														</a> 
													</div>
											...
											<td>{{ $smilyImage }}</td>
											<td>
												<div class="text-semibold">{{ setStringLimit($oReview->review_title, '25') }}</div>
												<div class="text-size-small text-muted">{{ setStringLimit($oReview->comment_text, '61') }}</div>
											</td>
											<td>
												<div class="media-left">
													<div class="">
													...
													<div class="text-muted text-size-small">{{ date('h:iA', strtotime($oReview->created)) }}</div>
												</div>
											</td>
											<td>
												<div>
													@if ($getComm > 0)
														<a style="cursor: pointer; color: #333333;" onclick="showCommentsPopup({{ $oReview->id }})">{{ $getComm }}</a>
													@else
														0
													@endif
												</div>
											...
											<td width="200">
												...
												@if($canWrite)
													<a href="javascript:void(0);" class="applyInsightTags btn btn-xs btn_white_table" action_name="review-tag" reviewid="{{ base64_url_encode($oReview->id) }}"> 
												...
												<div class="tdropdown">
													<ul class="icons-list">	
														...
														@endif
															...
															<button type="button" class="btn btn-xs btn_white_table ml20 dropdown-toggle" data-toggle="dropdown">
																<i class="icon-more2 txt_blue"></i>
															</button>
															<ul class="dropdown-menu dropdown-menu-right width-200">
																@if ($canWrite)
																	@if ($oReview->status == 1)
																		<li><a review_id='{{ $oReview->id }}' change_status = '0' title='Disapproved this review.' class='chg_status red'><i class='icon-file-locked'></i> Disapproved</a></li>
																	@else if ($oReview->status == 2)
																		<li><a review_id='{{ $oReview->id }}' change_status = '1' title='Approved this review.' class='chg_status green'><i class='icon-file-locked'></i> Approved</a></li>
																		<li><a review_id='{{ $oReview->id }}' change_status = '0' title='Disapproved this review.' class='chg_status red'><i class='icon-file-locked'></i> Disapproved</a></li>
																	@else
																		<li><a review_id='{{ $oReview->id }}' change_status = '1' title='Approved this review.' class='chg_status green'><i class='icon-file-locked'></i> Approved</a></li>
																	@endif
																	
																	<li><a href="javascript:void(0);" class="applyInsightTags" action_name="review-tag" reviewid="{{ base64_url_encode($oReview->id) }}" ><i class="icon-file-locked"></i> Apply Tags</a></li>
																	<li><a href="javascript:void(0);" class="displayReview" action_name="review-tag" tab_type="note" reviewid="{{ $oReview->id }}" review_time='{{ date("M d, Y h:i A", strtotime($oReview->created)) }}( {{ timeAgo($oReview->created) }})'><i class="icon-file-locked"></i> Add Notes</a></li>
																}
																<li><a href="javascript:void(0);" class="showReviewPopup" tab_type="info" reviewid="{{ $oReview->id }}" ><i class="icon-file-locked"></i> View Review Popup</a></li>

																<li><a target="_blank" href="{{ base_url('admin/brandboost/reviewdetails/' . $oReview->id) }}"><i class="icon-file-locked"></i> View Review</a></li>
																@if ($canWrite)
																	@if ($oReview->review_type == 'text')
																		<li><a href="javascript:void(0);" class="editReview" reviewid="{{ $oReview->id }}"><i class="icon-gear"></i> Edit</a></li>
																	@else
																		<li><a href="javascript:void(0);" class="editVideoReview" reviewid="{{ $oReview->id }}"><i class="icon-pencil"></i> Edit</a></li>
																	@endif
																	<li><a href="javascript:void(0);" class="deleteReview" reviewid="{{ $oReview->id }}" ><i class="icon-trash"></i> Delete</a></li>
																@endif
															</ul>
														</li>
													</ul>
