    ...
    <div class="page_header">
        <div class="row">
            ...
            <div class="col-md-5 text-right btn_area">
                <a href="{{ base_url() }}admin/settings/?t=notify" class="btn light_btn ml10"><i class="icon-cog"></i><span> &nbsp;  Settings</span> </a>
            </div>
        </div>
    </div>
    <!--&&&&&&&&&&&& PAGE HEADER END&&&&&&&&&&-->
            ...
            <div class="panel panel-flat">
                <div class="panel-heading">
                    ...
                    <div class="heading-elements">
                        ...
                        <a href="javascript:void(0);" class="txt_blue2 brig2 mr20 pr20 viewAllNotification"><i class=""><img width="16" src="{{ base_url() }}assets/images/icon_check_blue_double.png"/></i>&nbsp;Mark all read</a>
                        <a href="#" class="txt_green"><i class=""><img width="16" src="{{ base_url() }}assets/images/icon_check_green.png"/></i>&nbsp;Resolve Selected</a>
                    </div>
            ...
            if (!empty($notifications_data)) {
                foreach ($notifications_data as $notification) {
                    ...
                    <div class="notificatins_box" style="background-color: {{ $backgroundColor }}!important">
                        <div class="row">
                            <div class="col-md-7">
                                <div class="media-left"><img class="notif_icon" width="36" src="{{ base_url() }}assets/images/{{ $icon }}" /></div>
                                <div class="media-left"><p class="txt_dark">{!! $notification->message !!}</p></div>
                                @if (!empty($notification->link))
                                    <div class="media-left"><a href="javascript:void(0);" class="txt_blue2 readNotification" data-notifyid="{{ $notification->id }}" data-redirect="{{ $notification->link }}">Learn more <i class="icon-arrow-right13"></i></a></div>
                                @endif
                            </div>
                            <div class="col-md-5 text-right">
                                <div class="media-right"><p class="txt_grey">{{ $notification->firstname }} {{ $notification->lastname }}, {{ timeAgoNotification($notification->created) }}</p></div>
...
<script type="text/javascript">
            ...
            if (val.length === 0) {
                ...
                deleteConfirmationPopup(
                        ...
                        function () {
                            ...
                            $('.overlaynew').show();
                            $.ajax({
                                url: "{{ base_url('admin/notifications/delete_multipal_notification') }}",
                                type: "POST",
    ...
    $('.daterange-ranges-notification').daterangepicker(
            {
                ...
                endDate: moment(),
                minDate: '01/01/2018',
                maxDate: '{{ date('m/d/Y', strtotime("+1 days")) }}',
                //dateLimit: { days: 60 },
                ranges: {
                    'Today': [moment(), moment()],
            ...
            function (start, end) {
                ...
                $.ajax({
                    url: "{{ base_url('admin/notifications/getNotificationFilterDate') }}",
                    type: "POST",
    ...
    $.ajax({
        url: "{{ base_url('admin/notifications/getNotificationData') }}",
        type: "POST",
