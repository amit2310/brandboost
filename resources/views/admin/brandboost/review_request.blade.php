		...
		<div class="col-lg-12">
			...
			<div class="panel panel-flat">
				<div class="panel-heading" style="box-shadow:none">
					<h6 class="panel-title"><?php echo count($oRequest); ?> Review Requests</h6>
							...
							<?php
									...
									<tr id="append-<?php echo $data->trackinglogid; ?>" class="selectedClass">
								        ...
								        <div class="media-left">
								          ...
								          <div class="text-muted text-size-small"><?php 
								          if(!empty($data->brand_desc)) {
								          	echo setStringLimit($data->brand_desc, 20);
								          }
								          else {
								          	echo displayNoData();
								          }
								        ...
								        <div class="media-left">
								          <div class="pt-5"><a href="javascript:void();" class="text-default text-semibold"><?php echo ucfirst($data->tracksubscribertype); ?></a></div>
								        </div></td>
										...
										<td>
											<div class="media-left">
												<div class="pt-5"><span class="text-default text-semibold"><?php echo dataFormat($data->requestdate); ?></span></div>
										...
										<td class="text-center">
											<div class="tdropdown">
											<?php echo $data->subscriberstatus == 1 ? '<i class="icon-primitive-dot txt_green fsize16"></i>' : '<i class="icon-primitive-dot txt_red fsize16"></i>'; ?>
											<a class="text-default text-semibold bbot dropdown-toggle" data-toggle="dropdown"><?php echo $data->subscriberstatus == 1 ? ' Active' : ' Inactive'; ?></a>
											<ul style="right: 0;" class="dropdown-menu dropdown-menu-right status">
												<?php
													if ($data->subscriberstatus == 1) {
														echo "<li><a subscriberId='" . $data->subscriberid . "' change_status = '0' class='chg_status'><i class='icon-primitive-dot txt_red'></i> Inacive</a></li>";
														} else {
														echo "<li><a  subscriberId='" . $data->subscriberid . "' change_status = '1' class='chg_status'><i class='icon-primitive-dot txt_green'></i> Active</a></li>";
													}
												?>
											</ul>
											</div>
											...
											<ul class="icons-list">
												<li class="dropdown">
													...
													<ul class="dropdown-menu dropdown-menu-right">
														<?php
															if ($data->subscriberstatus == 1) {
																echo "<li><a subscriberId='" . $data->subscriberid . "' change_status = '0' class='chg_status'><i class='icon-file-locked'></i> Inacive</a></li>";
																} else {
...
<script type="text/javascript">
	...
	$(document).on('click', '#addOnSiteReview', function() {
		window.location.href = "<?php echo base_url(); ?>admin/brandboost/addreview/";
	});
