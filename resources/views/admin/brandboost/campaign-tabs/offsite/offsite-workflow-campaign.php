...
<div class="tab-pane <?php echo $emailWorkflow; ?>" id="right-icon-tab3">
		...
		<div class="timeline-container"> 
			...
			<?php
				...
				foreach ($eventsData as $key => $eventData) {
					...
					$currentStat = ($eventNo == 1) ? 'main' : 'reminder';
					...
					if (count($campaignArray) > 0) {
						foreach ($campaignArray as $campaignData) {
							...
							<div class="timeline-row post-full">
								<div class="timeline-icon"> <a data-toggle="modal" data-target="#addnewaction" pre_event_id="<?php echo $previousEventID; ?>" current_event_id="<?php echo $eventData->id; ?>" class="circle_plus" href="javascript:void(0);"><i class="icon-plus3"></i></a> </div>
							</div>
								...
								<div class="timeline-content">
									<div class="panel border-left-lg gery_bkg border-left-primary">
										<div class="panel-body">
											<div class="row">
												<div class="col-md-7">
													<h6 class="no-margin-top"><a href="#"><?php echo $eventNo; ?>. <?php echo ($eventNo == 1) ? 'Review Request Email' : 'Review Request Reminder'; ?> : Offsite invitation Sent</a></h6>
								...
								<div class="timeline-content">
									<div class="panel gery_bkg border-right-lg border-right-primary">
										<div class="panel-body">
											<div class="row">
												<div class="col-md-7">
													<h6 class="no-margin-top"><a href="task_manager_detailed.html"><?php echo $eventNo; ?>. <?php echo ($eventNo == 1) ? 'Review Request SMS' : 'Review Request Reminder'; ?> : Offsite invitation Sent</a></h6>
													<p class="mb-15"><?php echo count($subscribersData); ?> Contacts Added </p>
													<?php 
												...
												<div class="col-md-5 text-right">
													<a data-toggle="modal" data-target="#sms_modal" class="btn bl_cust_btn edit_email_template editSmsTemplate" campaign_id="<?php echo $campaignData->id; ?>" id="editSmsTemplate_<?php echo $campaignData->id; ?>" style="cursor: pointer;" ><i class="fa fa-edit"></i> </a> 
													<a data-toggle="modal" data-target="#sms_preview_modal" class="btn green_cust_btn" ><i class="fa fa-eye"></i> </a>
													<?php if($canWriteAna): ?>
													<button class="btn viewstats" campaign_id="<?php echo $campaignData->id; ?>"><i class="fa fa-pie-chart"></i></button>
													<?php endif; ?>
													<button class="btn red" onclick="javascript:delete_campaign('<?php echo $campaignData->id; ?>');"><i class="fa fa-trash"></i></button>
			...
			<div class="timeline-row post-full">
				<div class="timeline-icon"> <a data-toggle="modal" data-target="#addnewaction" class="circle_plus" href="#"><i class="icon-plus3"></i></a> </div>
			</div>
	...
	<div class="col-md-12 text-right">
		<a href="javascript:void(0);" targetName="Requires Attention" class="btn dark_btn mt10 continueStepOffsite" data-brandid="<?php echo $brandboostID;?>">
			Continue
						...
						<div class="tab-pane" id="SMSRequest">
							<div class="row">
								<?php foreach($smsTemplate as $smsTemp){ ?>
									<div class="col-md-6">
										<div template_id="<?php echo $emailTemp->id; ?>" class="panel bg-primary actionpanel openSmsTemplate">
											<div class="panel-body">
												<div class="media-left"> <img src="<?php echo site_url('assets/images/email_icon2.png'); ?>" style="width: 40px;"> </div>
												<div class="media-left">
								...
								<div class="col-md-12 txt_inp_grp">
									<form  method="post" name="eventFrmUpdate" class="eventFrmUpdate">
										<div style="float:left; width: 100%;">
											...
											<input name="event_id" id="event_id" value="201" type="hidden">
					...
					<div class="row">
						<div class="col-md-5">
							<div class="temp_left_option p15">
								...
								<div class="form-group">
									<label>Body: </label>
									<textarea class="form-control textareabot" id="campaignTempBody" name="campaignTempBody"  placeholder="A collection of textile samples lay spread out on the table.."></textarea>
								</div>
								<div class="form-group">
							...
							<div style="margin-top: 0px;" class="temp_left_option p15">
								<div class="form-group">
									...
									<select class="form-control" id="smsTempId" name="smsTempId">
										...
										<?php foreach($smsTemplate as $smsTemp){ ?>
											<option value="<?php echo $smsTemp->id; ?>"><?php echo $smsTemp->template_name; ?></option>
										<?php } ?>
									</select>
								</div>
	...
	$(document).ready(function(){
			...
			$.ajax({
				...
				success: function (data) {
					if(data.status == 'success'){
						...
						$('#campaignTempBody').val(data.emailContent);
		...
		$(document).on("click", ".openEmailTemplate", function () {
			...
			$.ajax({
				...
				type: "POST",
				data: {action : 'createEvent', templateId : templateId, campaign_type : 'Email'},
				dataType: "json",
				success: function (data) {
					if(data.status == 'success'){
		...
		$('#updateEmailCampaign').on('click', function (e) {
            ...
            $.ajax({
                ...
                type: "POST",
                data: {template_subject: templateSubject, template_greeting: templateGreeting, template_introduction: templateIntroduction, template_heading: templateHeading, template_content: templateContent, template_id: templateId, campaign_id: campaignId},
				dataType: "json",
                success: function (data) {
                    if (data.status == 'success') {
