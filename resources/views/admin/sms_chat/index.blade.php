...
@section('contents')
    ...
    <style>
        ...
        .chat_search_icon input[type="text"] {
            width: 222px;
            border: none;
            border: none;
            height: 20px;
        ...
        .chat_search_icon button[type="submit"] {
            width: 20px;
            height: 20px;
            border: none;
            background: none;
        ...
        .chat_search_icon button[type="submit"] img {
        ...
        .smsContainer .media_file {
            width: 250px;
        }

        .chatbox_new .img-circle {
            margin-top: 5px;
        }

        .chat_user_list span.icons.fl_letters_gray {
    ...
    <div class="content" id="SMSonly">
        ...
        <div class="page_header">
            ...
            <button style="float: right; margin-bottom: -10px" id="addBrandboost"
                    class="btn bl_cust_btn btn-default dark_btn ml20 ConversationsIcon2" type="button"><i
                    class="icon-plus3"></i> New Conversation
            </button>
        </div>
        <!--&&&&&&&&&&&& PAGE HEADER END &&&&&&&&&&-->
        ...
        <div class="row" id="mainContainer">
            <div class="col-md-3 pr6 firstDiv">
                        ...
                        <div class="p20 pt-15 pb-15">
                            ...
                            <div class="tdropdown ml0">
                                <a style="margin:0!important;" class="dropdown-toggle fsize12 txt_grey"
                                   data-toggle="dropdown" aria-expanded="false"><span class="t_sms_main"
                                ...
                                <ul style="right: 0px!important; margin-top: 25px; left: -20px;"
                                    ...
                                    <li><strong><a class="active" href="javascript:void(0)"><img class="small"
                                                ...
                                                <b class="smallbr">All ({{ $activeChatCount }})</b> </a></strong></li>
                                    ...
                                    <li><strong><a href="javascript:void(0)"><img class="small"
                                                                                  src="/assets/images/cd_icon4.png"/><span
                                                    class="f_link">Favorite ({{ $favouriteUserData->count() }})</span>
                                            </a></strong></li>
                                    <li><strong><a href="javascript:void(0)"><img class="small"
                                                                                  src="/assets/images/cd_icon5.png"/><span
                                                    class="c_link">Contacts({{ $totalSubscriber }})</span></a></strong>
                                    </li>
                            ...
                            <div class="tdropdown ml0 pull-right">
                                <a style="margin:0!important;" class="dropdown-toggle fsize12 txt_grey"
                                   data-toggle="dropdown" aria-expanded="false"><span class="f_sms_main"
                                ...
                                <ul style="margin-top: 25px; right: -20px;"
                                    ...
                                    <li class="f_new"><strong><a class="active"
                                                                 href="javascript:void(0)">Newest </a></strong></li>
                                    <li style="display: none" class="f_wait"><strong><a href="javascript:void(0)">Waiting
                                                longest </a></strong></li>
                        ...
                        <div class="p20 btop pt-15 pb-15">
                            <div style=" width:100%;" class="chat_search_icon">
                                <input style="width: calc(100% - 22px);"
                                       id="MainsearchSmsChatMsg" class="MainsearchSmsChatMsg" type="text"
                                       name="adSearch" value="" placeholder="Search">

                                <input style="width: calc(100% - 22px); display:none"
                                       id="MainContactsearch" class="MainContactsearch" type="text"
                                       name="MainContactsearch" value="" placeholder="Search">

                                <input style="display:none;width: calc(100% - 22px)" type="text" name="SmsContactBox"
                                       placeholder="Search" id="SmsContactBox" value="">
                ...
                <div id="InitalSms">
                    ...
                    <!--++++++++++++ Active chat list +++++++++++++++-->
                    <div id="a_list_sms" class="panel-body p0 br5 mb10 chat_user_list activeChat a_list"
                    ...
                    <!--++++++++++++ favourite chat list +++++++++++++++-->
                    <div class="panel-body p0 br5 mb10 chat_user_list favouriteChatlist f_list"
                ...
                <!-- Contact list -->
                <div class="panel-body p0 br5 mb10 chat_user_list subs_list c_list" style="background-image:none;
                @if (!empty($defaultNumber))
                    display:none;
                @else
                    display:block;
                @endif
                    ...
                    @endphp
                    <div id="chatBx_{{ $userData->phone }}" class="media chatbox_new bkg_white
                    @if ($count == 1)
                    {{ 'mb10' }}
                    @endif
                        " style="
                    ...
                    @endif
                        ">
                        <input type="hidden" id="userImg_{{ $userData->phone }}" value="">
                        <a href="javascript:void(0);" class="media-link bbot getChatDetails"
                           subscriberId="{{ $userData->id }}" phone_no="{{ $userData->phone }}">
                            <div
                                class="media-left">{!! showUserAvtar($avatar, $userData->firstname, $userData->lastname, 28, 28, 12) !!}
                                <span subscriberId="{{ $userData->phone }}"><i
                                        class="fa fa-star star_icon {{ $favUser > 0 ? 'yellow' : '' }}"></i></span>
                            </div>

                            <div class="media-body">

                                <span class="fsize12 txt_dark">{{ phoneNoFormat($userData->phone) }}</span>

                                <span class=""><span
                                        style="float: left; width: 100%; font-weight:300!important; color: #6a7995 !important; font-size: 12px; margin-bottom: 3px; ">
            Assigned to:&nbsp;<!--+1(359) 569-6585-->{{ phoneNoFormat($userData->phone) }}</span></span>

                                <span class="slider-phone contacts txt_dark"
                                      style="margin:0px; font-weight:bold; font-size:12px!important">{{ $userData->email }}</span>
                                <span class="contacts text-size-small txt_blue"
                                      style="margin:0px;display:none">{{ $userData->phone }}</span>
    ...
    <script>
        $(document).ready(function () {
            $('#SMSonly .c_link').html('Contacts({{ $contactCount }})');
        ...
        function updatReadStatus(SubscriberPhone, token) {
            $.ajax({
                url: "{{ base_url('admin/Chat/setSmsReadstatus') }}",
                type: "POST",
        ...
        function showSMSChatData(userId, SubscriberPhone, clickvalue = null) {
            ...
            $.ajax({
                url: "{{ base_url('admin/smschat/getSubsinfo') }}",
                type: "GET",
        ...
        $('body').on('click', '.smsContainer .sendMsg', function () {
            ...
            if (userPhoneNo == '') {
                ...
                setTimeout(function () {
                    $('#SMSonly .messageContent').removeClass('borderClass');
                }, 50);
                ...
                for (var i = 0; i < messageSmilies.length; i++) {
                    ...
                    if (smiliesMapSMSChat[messageSmileyLower]) {
                        messageText = messageText.replace(messageSmiley, "<img style='width:auto; height:auto;' src='{{ base_url() }}assets/img-smile/" + smiliesMapSMSChat[messageSmileyLower] + ".gif' alt='smiley' />");
                    }
                }
                ...
                if (flag == 0) {
                    if (document.getElementById("em_new_number").value == '1') {
                        ...
                        $.ajax({
                            url: "{{ base_url('admin/subscriber/add_contact') }}",
                            type: "POST",
                ...
                $.ajax({
                    url: "{{ base_url('admin/smschat/sendMsg') }}",
                    type: "POST",
                    data: {
                        'phoneNo': userPhoneNo,
        ...
        $(document).on('change', '.smsContainer #mmsFile', function (e) {
            ...
            var aToken = Number(chatTo) + Number('{{ $loginUserData->mobile }}');

            if (Number(chatTo) > Number('{{ $loginUserData->mobile }}')) {
                var sToken = Number(chatTo) - Number('{{ $loginUserData->mobile }}');
            } else {
                var sToken = Number('{{ $loginUserData->mobile }}') - Number(chatTo);
            }
            var newToken = aToken + 'n' + sToken;
                        ...
                        setTimeout(function () {
                            ...
                            if (fileext[0] == 'mp4' || fileext[0] == 'webm' || fileext[0] == 'ogg') {
                                ...
                                $.ajax({
                                    ...
                                    url: "{{ base_url('admin/smschat/sendMMS') }}",
        ...
        function showResult(str) {
            ...
            xmlhttp.open("GET", "{{ base_url('admin/smschat/livesearch') }}?q=" + str, true);
            xmlhttp.send();
        }
        ...
        $(document).on('click', '#SMSonly .c_link', function () {
            ...
            $('#SMSonly .t_sms_main').html('Contacts({{ $contactCount }})');
        ...
        $(document).on('click', '#SMSonly .f_link', function () {
            ...
            $('#SMSonly .w_list').hide();
            $('#SMSonly .f_list').show();
            $('#SMSonly .t_sms_main').html('Favorite({{ $favouriteUserData->count() }})');
        ...
        $(document).on('click', '#SMSonly .smallbr', function () {
            ...
            $('#SMSonly .c_list').hide();
            $('#SMSonly .a_list').show();
            $('#SMSonly .t_sms_main').html('All ({{ $activeChatCount }})');
            $('#SmsContactBox').hide();
        ...
        $(document).ready(function () {
            $(document).on("keyup", ".MainsearchSmsChatMsg", function () {
                $.ajax({
                    url: "{{ base_url('admin/smschat/getSearchSmsListByinput') }}",
                    type: "POST",
        ...
        //  ######### Live Search ############ //
        @if ($defaultNumber != "")
        showNoteslisting({{ $defaultNumber }});
        showSMSChatData('{{ $DefaultsubscriberId }}', '{{ $defaultNumber }}');
        @else
        showSMSChatData('{{ $subscriberId }}', '{{ $defaultUserNumber }}');
        showNoteslisting({{ $defaultUserNumber }});
        @endif
    </script>
    <script>
        $(document).ready(function () {
