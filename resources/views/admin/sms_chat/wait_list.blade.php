...
if( $value->to!='' && $value->from!='' )
{
   ...
   $value->from  = phone_display_custom_helper($value->from);
	...
	<div  id="active_chat_box" class="activityShow <?php echo $count; ?> media chatbox_new bkg_white <?php if($count == 1){ echo 'mb10'; } ?> sms_twr_<?php echo $phoneNumber; ?>" style="<?php  if($count > 7) {  echo "display:block";   }  if($count == 1){ echo 'box-shadow:0 2px 4px 0 rgba(1, 21, 64, 0.06)!important; border-radius:0 0 5px 5px'; } if($count==2) {  echo 'border-radius:5px 5px 5px 5px';  }?>"> 
		<a formatedNumber="<?php echo mobileNoFormat($phoneNumber); ?>" href="javascript:void(0);" class="media-link bbot <?php if($count!=1){ echo 'bbot'; } ?> getChatDetails <?php echo $count == 0 ? 'activechat' : ''; ?>" subscriberId="<?php echo $usersdata->id; ?>" phone_no="<?php echo $usersdata->phone; ?>" rewId="<?php echo $value->from; ?>" <?php if($showRed){ ?>wait="yes" token="<?php echo $value->token; ?>"<?php } ?>>
