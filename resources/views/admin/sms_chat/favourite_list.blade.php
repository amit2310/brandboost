<?php
 foreach($favouriteUserData as $key=>$value)
 {
	  ...
	  $favUser=1;
?>
	<div id="active_chat_box" class="<?php echo $count; ?> media chatbox_new bkg_white <?php if($count == 1){ echo 'mb10'; } ?>" style="<?php  if($count > 7) {  echo "display:block";   }  if($count == 1){ echo 'box-shadow:0 2px 4px 0 rgba(1, 21, 64, 0.06)!important; border-radius:0 0 5px 5px'; } if($count==2) {  echo 'border-radius:5px 5px 5px 5px';  }?>"> 
		<a formatedNumber="<?php echo phoneNoFormat($phoneNumber); ?>" href="javascript:void(0);" class="media-link bbot <?php if($count!=1){ echo 'bbot'; } ?> getChatDetails <?php echo $count == 0 ? '' : ''; ?>" subscriberId="<?php echo $userDataDetail->id; ?>" phone_no="<?php echo $phoneNumber; ?>">
            ...
            Assigned to:&nbsp;<!--+1(359) 569-6585--><?php echo phoneNoFormat($phoneNumber); ?></span></span>
				...
				style="margin:0px; font-weight:bold; font-size:12px!important"><?php echo $userDataDetail->email ;?><?php //echo $userMessage; ?></span> 
