...
<span class="reviews visible-xs">Reviews</span>
	...
	<div class="bb_white_box {{ $bgClassName }}" style="padding:0 30px;">
			...
			<div id="bb_site_rating_area" style="display:none;">
				...
				<div class="bb_progress_bar_sec">
					...
					<div class="bb_progress_bar_inner">
						...
						<div class="bb_progress">
							<div class="bb_progress_grey">
								<div class="bb_progress_status red width15" style="width:{{ $twoStarSiteRatingsPer }}%;" title="{{ $twoStarSiteRatings }}"> </div>
							</div>
						</div>
						<div class="bb_clear"></div>
					</div>
		...
		<div class="bb_tab_box bb_custom_bc">
			<ul>
				<li><a class="active bb_custom_fc bb_product_reviews" href="javascript:void(0);"><span class="hidden-xs">Product</span> Reviews ({{ sizeof($aReviews) - sizeof($aSiteReviews) - sizeof($aServiceReviews) }})</a></li>
							...
							<a href="javascript:void(0)">
								@foreach ($brandImgArray as $imageData)
									@if ($reviewData['media_url'] != '' && $imageData['media_url'] != '' && $imageData['media_type'] == 'image'):
										<img src="https://s3-us-west-2.amazonaws.com/brandboost.io/campaigns/{{ $imageData['media_url'] }}" alt="" class="bb_img_enlagre bb_normal_image">';
									@endif
								@endforeach
							</a>
                    	</div>
						...
						@endphp
							...
							<div class="bb_inner_reply">
								<div class="bb_comment_header_small">
									...
									<div class="bb_fleft">
										<p class="bb_para"><span class="bb_dot"><i class="fa fa-circle"></i></span> <span class="bb_thingrey bb_custom_fc">{{ timeAgo(date('F d, Y', strtotime($aComment['created']))) }}</span></p>
									</div>
									<div class="bb_clear"></div>
								</div>
						...
						@endphp
							<div class="bb_inner_reply" style="margin-left:40px;">
								<div class="bb_comment_header_small">
									...
									<div class="bb_fleft">
										<p class="bb_para"><span class="bb_dot"><i class="fa fa-circle"></i></span> <span class="bb_thingrey bb_custom_fc">{{ timeAgo(date('F d, Y', strtotime($cComment['created']))) }}</span></p>
									</div>
									<div class="bb_clear"></div>
								</div>
								<div class="text_section_reply">
								...
								<div class="bb_comment_area_reply">
									<span class="bb_custom_fc bbpw_comment_like_{{ $cComment['id'] }}">{{ $oCommentLike['like'] }}</span>&nbsp;
									...
									<span class="bb_custom_fc bbpw_comment_dislike_{{ $cComment['id'] }}">{{ $oCommentLike['dislike'] }}</span>&nbsp;
